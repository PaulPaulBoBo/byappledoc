...
@property (nonatomic, readwrite, strong) GBApplicationSettingsProvider *settings;
@property (nonatomic, strong) NSMutableArray *additionalInputPaths;
@property (nonatomic, strong) NSMutableArray *ignoredInputPaths;
@property (nonatomic, weak) NSString *logformat;
@property (nonatomic, weak) NSString *verbose;
@property (nonatomic, assign) BOOL templatesFound;
...
- (int)application:(DDCliApplication *)app runWithArguments:(NSArray *)arguments {
    ...
    @try {
        ...
        GBLogWarn(@"Initializing...");
        GBStore *store = [[GBStore alloc] init];
        GBAbsoluteTime startTime = GetCurrentTime();
        
        GBLogWarn(@"Parsing source files...");
        ...
        GBLogWarn(@"Finished parsing in %ldms.\n", timeForParsing);
        
        GBLogWarn(@"Processing parsed data...");
        ...
        GBLogWarn(@"Finished processing in %ldms.\n", timeForProcessing);
        
        GBLogWarn(@"Generating output...");
        ...
        GBLogWarn(@"Finished generating in %ldms.\n", timeForGeneration);
        
        NSUInteger timeForEverything = timeForParsing + timeForProcessing + timeForGeneration;
        GBLogWarn(@"Finished in %ldms.", timeForEverything);
        GBLogWarn(@"Parsing:   %ldms (%ld%%)", timeForParsing, timeForParsing * 100 / timeForEverything);
        GBLogWarn(@"Processing:%ldms (%ld%%)", timeForProcessing, timeForProcessing * 100 / timeForEverything);
        GBLogWarn(@"Generating:%ldms (%ld%%)", timeForGeneration, timeForGeneration * 100 / timeForEverything);
    }
    @catch (NSException *e) {
    ...
    GBLogWarn(@"Exiting with result %d (reported result was %ld - higher than %d)...", result, kGBLogBasedResult, self.settings.exitCodeThreshold);
    return result;
}
...
- (void)deleteContentsOfOutputPath {
    ...
    if (!self.settings.cleanupOutputPathBeforeRunning) {
        return;
    }
...
- (void)validateSettingsAndArguments:(NSArray *)arguments {
    ...
    if ([self.settings.projectName length] == 0) {
        [NSException raise:@"--%s argument or global setting is required!", kGBArgProjectName];
    }
    if ([self.settings.projectCompany length] == 0) {
        [NSException raise:@"--%s argument or global setting is required!", kGBArgProjectCompany];
    }
    ...
    if ([self.settings.outputPath length] == 0) {
        ...
        if (self.settings.warnOnMissingOutputPathArgument) {
            NSLog(@"WARN:--%s argument or global setting not given, will output to current dir '%@'!\n", kGBArgOutputPath, self.settings.outputPath);
        }
    }
    ...
    if (self.settings.createDocSet && [self.settings.companyIdentifier length] == 0) {
        ...
        if (self.settings.warnOnMissingCompanyIdentifier) {
            NSLog(@"WARN:--%s argument or global setting not given, but creating DocSet is enabled, will use '%@'!\n", kGBArgCompanyIdentifier, self.settings.companyIdentifier);
        }
    }
    ...
    [self.settings.includePaths enumerateObjectsUsingBlock:^(NSString *userPath, BOOL *stop) {
        ...
        if (![self.fileManager fileExistsAtPath:path]) {
            NSLog(@"WARN:--%s path '%@' doesn't exist, ignoring!\n", kGBArgIncludePath, userPath);
        }
    }];
    ...
    if ([self.settings.indexDescriptionPath length] > 0) {
        ...
        if (![self.fileManager fileExistsAtPath:path isDirectory:&isDir])
            ...
            NSLog(@"WARN:--%s path '%@' is a directory, file is required, ignoring!\n", kGBArgIndexDescPath, self.settings.indexDescriptionPath);
    }
    ...
    if (self.settings.useSingleStarForBold) {
        NSLog(@"WARN:--%s may cause incompatibility with Markdown (* unordered lists etc.)", kGBArgUseSingleStar);
    }
}
...
- (void)injectProjectSettingsFromArguments:(NSArray *)arguments {
    ...
    [arguments enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSString *argument, NSUInteger idx, BOOL *stop) {
        ...
        BOOL dir;
        if (![self.fileManager fileExistsAtPath:path isDirectory:&dir]) {
            return;
        }
        ...
        if ([[filename pathExtension] isEqualToString:@"plist"]) {
            ...
            [self injectSettingsFromSettingsFile:filename usingBlock:^BOOL(NSString *option, id *value, BOOL *stop) {
                ...
                if ([option isEqualToString:GBArgToNSString(kGBArgInputPath)]) {
                    for (__strong NSString *inputPath in *value) {   inputPath = [self combineBasePath:plistPath withRelativePath:inputPath];   [self.additionalInputPaths addObject:inputPath];
                    }
                    return NO;
...
- (void)overrideSettingsWithGlobalSettingsFromPath:(NSString *)path {
    ...
    [self injectSettingsFromSettingsFile:filename usingBlock:^(NSString *option, id *value, BOOL *stop) {
        if ([option isEqualToString:GBArgToNSString(kGBArgTemplatesPath)]) {
            NSLog(@"WARN:Found unsupported --%@ option in global settings file '%@'!\n", option, userPath);
            return NO;
        }
        if ([option isEqualToString:GBArgToNSString(kGBArgInputPath)]) {
            NSLog(@"WARN:Found unsupported --%@ option in global settings '%@'!\n", option, userPath);
            return NO;
        }
        return YES;
    }];
...
- (void)setOutput:(NSString *)path {
    self.settings.outputPath = [self standardizeCurrentDirectoryForPath:path];
}

- (void)setDocsetInstallPath:(NSString *)path {
    self.settings.docsetInstallPath = [self standardizeCurrentDirectoryForPath:path];
}

- (void)setXCRunPath:(NSString *)path {
    self.settings.xcrunPath = [self standardizeCurrentDirectoryForPath:path];
}

- (void)setInclude:(NSString *)path {
    [self.settings.includePaths addObject:[self standardizeCurrentDirectoryForPath:path]];
}

- (void)setIndexDesc:(NSString *)path {
    self.settings.indexDescriptionPath = [self standardizeCurrentDirectoryForPath:path];
}

- (void)setTemplates:(NSString *)path {
    self.settings.templatesPath = [self standardizeCurrentDirectoryForPath:path];
}
...
- (void)setProjectName:(NSString *)value {
    self.settings.projectName = value;
}

- (void)setProjectVersion:(NSString *)value {
    self.settings.projectVersion = value;
}

- (void)setProjectCompany:(NSString *)value {
    self.settings.projectCompany = value;
}
...
- (void)setCleanOutput:(BOOL)value {
...
- (void)setNoCleanOutput:(BOOL)value {
    self.settings.cleanupOutputPathBeforeRunning = !value;
}

- (void)setNoCreateHtml:(BOOL)value {
    [self setCreateHtml:!value];
}

- (void)setNoCreateDocset:(BOOL)value {
    [self setCreateDocset:!value];
}

- (void)setNoFinalizeDocset:(BOOL)value {
    [self setFinalizeDocset:!value];
}

- (void)setNoInstallDocset:(BOOL)value {
    [self setInstallDocset:!value];
}

- (void)setNoPublishDocset:(BOOL)value {
    [self setPublishDocset:!value];
}

- (void)setCrossrefFormat:(NSString *)value {
    self.settings.commentComponents.crossReferenceMarkersTemplate = value;
}

- (void)setExplicitCrossref:(BOOL)value {
    self.settings.commentComponents.crossReferenceMarkersTemplate = value ? @"<%@>" :@"<?%@>?";
}

- (void)setNoExplicitCrossref:(BOOL)value {
    [self setExplicitCrossref:!value];
}

- (void)setExitThreshold:(int)value {
    self.settings.exitCodeThreshold = value;
}

- (void)setDocsSectionTitle:(NSString *)value {
    self.settings.docsSectionTitle = value;
}

- (void)setKeepIntermediateFiles:(BOOL)value {
    self.settings.keepIntermediateFiles = value;
}

- (void)setKeepUndocumentedObjects:(BOOL)value {
    self.settings.keepUndocumentedObjects = value;
}

- (void)setKeepUndocumentedMembers:(BOOL)value {
    self.settings.keepUndocumentedMembers = value;
}

- (void)setSearchUndocumentedDoc:(BOOL)value {
    self.settings.findUndocumentedMembersDocumentation = value;
}

- (void)setRepeatFirstPar:(BOOL)value {
    self.settings.repeatFirstParagraphForMemberDescription = value;
}

- (void)setPreprocessHeaderdoc:(BOOL)value {
    self.settings.preprocessHeaderDoc = value;
}

- (void)setPrintInformationBlockTitles:(BOOL)value {
    self.settings.printInformationBlockTitles = value;
}

- (void)setUseSingleStar:(BOOL)value {
    self.settings.useSingleStarForBold = value;
}

- (void)setMergeCategories:(BOOL)value {
    self.settings.mergeCategoriesToClasses = value;
}

- (void)setMergeCategoryComment:(BOOL)value {
    self.settings.mergeCategoryCommentToClass = value;
}

- (void)setKeepMergedSections:(BOOL)value {
    self.settings.keepMergedCategoriesSections = value;
}

- (void)setPrefixMergedSections:(BOOL)value {
    self.settings.prefixMergedCategoriesSectionsWithCategoryName = value;
}

- (void)setNoKeepIntermediateFiles:(BOOL)value {
    self.settings.keepIntermediateFiles = !value;
}

- (void)setNoKeepUndocumentedObjects:(BOOL)value {
    self.settings.keepUndocumentedObjects = !value;
}

- (void)setNoKeepUndocumentedMembers:(BOOL)value {
    self.settings.keepUndocumentedMembers = !value;
}

- (void)setNoSearchUndocumentedDoc:(BOOL)value {
    self.settings.findUndocumentedMembersDocumentation = !value;
}

- (void)setNoRepeatFirstPar:(BOOL)value {
    self.settings.repeatFirstParagraphForMemberDescription = !value;
}

- (void)setNoUseSingleStar:(BOOL)value {
    self.settings.useSingleStarForBold = !value;
}

- (void)setNoPreprocessHeaderdoc:(BOOL)value {
    self.settings.preprocessHeaderDoc = !value;
}

- (void)setNoPrintInformationBlockTitles:(BOOL)value {
    self.settings.printInformationBlockTitles = !value;
}

- (void)setNoMergeCategories:(BOOL)value {
    self.settings.mergeCategoriesToClasses = !value;
}

- (void)setNoMergeCategoryComment:(BOOL)value {
    self.settings.mergeCategoryCommentToClass = !value;
}

- (void)setNoKeepMergedSections:(BOOL)value {
    self.settings.keepMergedCategoriesSections = !value;
}

- (void)setNoPrefixMergedSections:(BOOL)value {
    self.settings.prefixMergedCategoriesSectionsWithCategoryName = !value;
}

- (void)setUseCodeOrder:(BOOL)value {
    self.settings.useCodeOrder = value;
}

- (void)setNoUseCodeOrder:(BOOL)value {
    self.settings.useCodeOrder = !value;
}

- (void)setWarnMissingOutputPath:(BOOL)value {
    self.settings.warnOnMissingOutputPathArgument = value;
}

- (void)setWarnMissingCompanyId:(BOOL)value {
    self.settings.warnOnMissingCompanyIdentifier = value;
}

- (void)setWarnUndocumentedObject:(BOOL)value {
    self.settings.warnOnUndocumentedObject = value;
}

- (void)setWarnUndocumentedMember:(BOOL)value {
    self.settings.warnOnUndocumentedMember = value;
}

- (void)setWarnEmptyDescription:(BOOL)value {
    self.settings.warnOnEmptyDescription = value;
}

- (void)setWarnUnknownDirective:(BOOL)value {
    self.settings.warnOnUnknownDirective = value;
}

- (void)setWarnInvalidCrossref:(BOOL)value {
    self.settings.warnOnInvalidCrossReference = value;
}

- (void)setWarnMissingArg:(BOOL)value {
    self.settings.warnOnMissingMethodArgument = value;
}

- (void)setWarnUnsupportedTypedefEnum:(BOOL)value {
    self.settings.warnOnUnsupportedTypedefEnum = value;
}

- (void)setNoWarnMissingOutputPath:(BOOL)value {
    self.settings.warnOnMissingOutputPathArgument = !value;
}

- (void)setNoWarnMissingCompanyId:(BOOL)value {
    self.settings.warnOnMissingCompanyIdentifier = !value;
}

- (void)setNoWarnUndocumentedObject:(BOOL)value {
    self.settings.warnOnUndocumentedObject = !value;
}

- (void)setNoWarnUndocumentedMember:(BOOL)value {
    self.settings.warnOnUndocumentedMember = !value;
}

- (void)setNoWarnEmptyDescription:(BOOL)value {
    self.settings.warnOnEmptyDescription = !value;
}

- (void)setNoWarnUnknownDirective:(BOOL)value {
    self.settings.warnOnUnknownDirective = !value;
}

- (void)setNoWarnInvalidCrossref:(BOOL)value {
    self.settings.warnOnInvalidCrossReference = !value;
}

- (void)setNoWarnMissingArg:(BOOL)value {
    self.settings.warnOnMissingMethodArgument = !value;
}

- (void)setNoWarnUnsupportedTypedefEnum:(BOOL)value {
    self.settings.warnOnUnsupportedTypedefEnum = !value;
}

- (void)setDocsetBundleId:(NSString *)value {
    self.settings.docsetBundleIdentifier = value;
}

- (void)setDocsetBundleName:(NSString *)value {
    self.settings.docsetBundleName = value;
}

- (void)setDocsetDesc:(NSString *)value {
    self.settings.docsetDescription = value;
}

- (void)setDocsetCopyright:(NSString *)value {
    self.settings.docsetCopyrightMessage = value;
}

- (void)setDocsetFeedName:(NSString *)value {
    self.settings.docsetFeedName = value;
}

- (void)setDocsetFeedUrl:(NSString *)value {
    self.settings.docsetFeedURL = value;
}
...
- (void)setDocsetPackageUrl:(NSString *)value {
    self.settings.docsetPackageURL = value;
}

- (void)setDocsetFallbackUrl:(NSString *)value {
    self.settings.docsetFallbackURL = value;
}

- (void)setDocsetPublisherId:(NSString *)value {
    self.settings.docsetPublisherIdentifier = value;
}

- (void)setDocsetPublisherName:(NSString *)value {
    self.settings.docsetPublisherName = value;
}

- (void)setDocsetMinXcodeVersion:(NSString *)value {
    self.settings.docsetMinimumXcodeVersion = value;
}

- (void)setDocsetPlatformFamily:(NSString *)value {
    self.settings.docsetPlatformFamily = value;
}

- (void)setDocsetCertIssuer:(NSString *)value {
    self.settings.docsetCertificateIssuer = value;
}

- (void)setDocsetCertSigner:(NSString *)value {
    self.settings.docsetCertificateSigner = value;
}

- (void)setDashPlatformFamily:(NSString *)value {
    self.settings.dashDocsetPlatformFamily = value;
}

- (void)setDocsetBundleFilename:(NSString *)value {
    self.settings.docsetBundleFilename = value;
}

- (void)setDocsetAtomFilename:(NSString *)value {
    self.settings.docsetAtomFilename = value;
}

- (void)setDocsetXmlFilename:(NSString *)value {
    self.settings.docsetXMLFilename = value;
}

- (void)setDocsetPackageFilename:(NSString *)value {
    self.settings.docsetPackageFilename = value;
}
...
#define PRINT_BOOL(v) (v ? @"YES" :@"NO")
    ...
    NSLog(@"Running for files in locations:\n");
    for (NSString *path in arguments) NSLog(@"- %@\n", path);
    NSLog(@"\n");
    
    NSLog(@"Settings used for this run:\n");
    ...
    for (NSString *path in self.settings.ignoredPaths) {
        NSLog(@"--%s = %@\n", kGBArgIgnorePath, path);
    }
    for (NSString *path in self.settings.excludeOutputPaths) {
...
#define PRINT_USAGE(short,long,arg,desc) [self printHelpForShortOption:short longOption:[NSString stringWithUTF8String:long] argument:arg description:desc]
    ...
    NSLog(@"Usage:%@ [OPTIONS] <paths to source dirs or files>\n", name);
    NSLog(@"\n");
    NSLog(@"PATHS\n");
    ...
    NSLog(@"\n");
    NSLog(@"==================================================================\n");
    NSLog(@"[b] boolean parameter, uses no value, use --no- prefix to negate.\n");
    NSLog(@"\n");
    NSLog(@"[*] indicates parameters accepting placeholder strings:\n");
    NSLog(@"- %@ replaced with --project-name\n", kGBTemplatePlaceholderProject);
    NSLog(@"- %@ replaced with normalized --project-name\n", kGBTemplatePlaceholderProjectID);
    NSLog(@"- %@ replaced with --project-version\n", kGBTemplatePlaceholderVersion);
    NSLog(@"- %@ replaced with normalized --project-version\n", kGBTemplatePlaceholderVersionID);
    NSLog(@"- %@ replaced with --project-company\n", kGBTemplatePlaceholderCompany);
    NSLog(@"- %@ replaced with --company-id\n", kGBTemplatePlaceholderCompanyID);
    NSLog(@"- %@ replaced with current year (format yyyy)\n", kGBTemplatePlaceholderYear);
    NSLog(@"- %@ replaced with current date (format yyyy-MM-dd)\n", kGBTemplatePlaceholderUpdateDate);
    NSLog(@"- %@ replaced with --docset-bundle-filename\n", kGBTemplatePlaceholderDocSetBundleFilename);
    NSLog(@"- %@ replaced with --docset-atom-filename\n", kGBTemplatePlaceholderDocSetAtomFilename);
    NSLog(@"- %@ replaced with --docset-package-filename\n", kGBTemplatePlaceholderDocSetPackageFilename);
    NSLog(@"\n");
    NSLog(@"==================================================================\n");
    NSLog(@"Find more help and tips online:\n");
    NSLog(@"- http://appledoc.gentlebytes.com/\n");
    NSLog(@"- http://tomaz.github.com/appledoc/\n");
    NSLog(@"\n");
    NSLog(@"==================================================================\n");
    NSLog(@"%@ uses the following open source components, fully or partially:\n", name);
    NSLog(@"\n");
    NSLog(@"- DDCli by Dave Dribin\n");
    NSLog(@"- CocoaLumberjack by Robbie Hanson\n");
    NSLog(@"- ParseKit by Todd Ditchendorf\n");
    NSLog(@"- RegexKitLite by John Engelhart\n");
    NSLog(@"- GRMustache by Gwendal Roué\n");
    NSLog(@"- Discount by David Parsons\n");
    NSLog(@"- Timing functions from Apple examples\n");
    NSLog(@"\n");
    NSLog(@"We'd like to thank all authors for their contribution!\n");
}

- (void)printHelpForShortOption:(NSString *)aShort longOption:(NSString *)aLong argument:(NSString *)argument description:(NSString *)description {
    while([aLong length] + [argument length] < 32) {
