...
@property (nonatomic, strong) id sectionID; //tmp for class's refid
...
- (BOOL)generateOutputWithStore:(id)store error:(NSError **)error {
    ...
    if (![self copyOrMoveSourceFilesToDocuments:error]) {
        return NO;
    }
    if (![self processInfoPlist:error]) {
        return NO;
    }
    if (![self processNodesXml:error]) {
        return NO;
    }
    if (![self processTokensXml:error]) {
        return NO;
    }
    if (![self indexDocSet:error]) {
        return NO;
    }
    if (![self removeTemporaryFiles:error]) {
        return NO;
    }
    return YES;
}
...
- (BOOL)processTokensXml:(NSError **)error {
    ...
    NSUInteger index = 1;
    if (![self processTokensXmlForObjects:self.documents type:@"doc" template:templatePath index:&index error:error]) {
        return NO;
    }
    if (![self processTokensXmlForObjects:self.classes type:@"cl" template:templatePath index:&index error:error]) {
        return NO;
    }
    if (![self processTokensXmlForObjects:self.categories type:@"cat" template:templatePath index:&index error:error]) {
        return NO;
    }
    if (![self processTokensXmlForObjects:self.protocols type:@"intf" template:templatePath index:&index error:error]) {
        return NO;
    }
    if (![self processTokensXmlForObjects:self.constants type:@"tdef" template:templatePath index:&index error:error]) {
        return NO;
    }
    if (![self processTokensXmlForObjects:self.blocks type:@"tdef" template:templatePath index:&index error:error]) {
        return NO;
    }
    return YES;
}
...
- (BOOL)removeTemporaryFiles:(NSError **)error {
    ...
    if (self.settings.keepIntermediateFiles) {
        return YES;
    }
...
- (void)addTokensXmlModelObjectDataForPropertySetterAndGetter:(GBModelBase *)method withData:(NSDictionary *)data toArray:(NSMutableArray *)members {
    ...
    if (![method isKindOfClass:[GBMethodData class]]) {
        return;
    }
