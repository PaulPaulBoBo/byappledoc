...
@property (nonatomic, strong) GBCommentsProcessor *commentsProcessor;
@property (nonatomic, strong) id currentContext;
@property (nonatomic, strong) GBStore *store;
@property (nonatomic, strong) GBApplicationSettingsProvider *settings;
...
- (void)processClasses {
    ...
    for (GBClassData *class in classes) {
        ...
        [self.store.methods addObjectsFromArray:class.methods.methods];
        ...
        BOOL isSuc = [self isCommentValid:class.comment];
        if (!isSuc) {
            [self.store.unDocClasses addObject:class];
        }
...
- (void)processCategories {
    ...
    for (GBCategoryData *category in categories) {
        ...
        BOOL isSuc = [self isCommentValid:category.comment];
        if (!isSuc) {
            [self.store.unDocCategories addObject:category];
        }
...
- (void)processProtocols {
    ...
    for (GBProtocolData *protocol in protocols) {
        ...
        BOOL isSuc = [self isCommentValid:protocol.comment];
        if (!isSuc) {
            [self.store.unDocProtocols addObject:protocol];
        }
...
- (void)processConstants {
    ...
    for (GBTypedefEnumData *enumData in constants) {
        ...
        BOOL isSuc = [self isCommentValid:enumData.comment];
        if (!isSuc) {
            [self.store.unDocConstants addObject:enumData];
        }
...
- (void)processBlocks {
    ...
    for (GBTypedefBlockData *blockData in blocks) {
        ...
        BOOL isSuc = [self isCommentValid:blockData.comment];
        if (!isSuc) {
            [self.store.unDocBlocks addObject:blockData];
        }
...
- (void)processMethodsFromProvider:(GBMethodsProvider *)provider {
    ...
    for (GBMethodData *method in methods) {
        ...
        BOOL isSuc = [self isCommentValid:method.comment];
        if(!isSuc) {
            [self.store.unDocMethods addObject:method];
        }
...
- (void)processParametersFromComment:(GBComment *)comment matchingMethod:(GBMethodData *)method {
    ...
    if ([parameters count] > 0) {
        ...
        if (method.includeInOutput) {
            GBLogXWarn(comment.sourceInfo, @"%@:%ld unknown parameter descriptions (%@) found for %@", comment.sourceInfo, [parameters count], description, method);
        }
    }
    ...
    for (GBCategoryData *category in categories) {
        ...
        if ([category.methods.methods count] > 0) {
            ...
            for (GBMethodSectionData *section in category.methods.sections) {
                ...
                if (self.settings.keepMergedCategoriesSections) {
                    if (self.settings.prefixMergedCategoriesSectionsWithCategoryName && !category.isExtension) {   NSString *template = self.settings.stringTemplates.objectPage[@"mergedPrefixedCategorySectionTitle"];   NSString *name = [NSString stringWithFormat:template, category.nameOfCategory, section.sectionName];   [classMethodProvider registerSectionWithName:name];
                    } else {   [classMethodProvider registerSectionWithName:section.sectionName];
                    }
                }
