    ...
    while (searchRange.length > 0) {
        ...
        if ([captured count] > 0) {
            ...
            if (([begin isEqualToString:@"@code"] && [end isEqualToString:@"@endcode"]) || [begin isEqualToString:end]) {
                NSDictionary *component = @{@"code":code, @"prefix":captured[2], @"begin":begin, @"postfix":captured[4], @"end":end, @"range":[NSValue valueWithRange:codeRange]};
...
- (NSArray*)linkComponentsInString:(NSString*)string withCodeComponents:(NSArray*)codeComponents {
    ...
    while (searchRange.length > 0) {
        ...
        if (found.length) {
            ...
            searchRange.length = stringLength - searchRange.location;
            NSDictionary *component = @{@"link":[string substringWithRange:found], @"range":[NSValue valueWithRange:found]};
