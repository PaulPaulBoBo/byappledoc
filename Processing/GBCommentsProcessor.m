...
@property (nonatomic, strong) GBComment *currentComment;
...
@property (nonatomic, strong) NSMutableDictionary *reservedShortDescriptionData;
@property (nonatomic, strong) GBSourceInfo *currentSourceInfo;
@property (nonatomic, strong) id lastReferencedObject;
...
- (BOOL)processAbstractBlockInString:(NSString *)string lines:(NSArray *)lines blockRange:(NSRange)blockRange shortRange:(NSRange)shortRange {
    ...
    NSRange range;
    @try {
        if(index.location < description.length) {
...
- (GBCrossRefData *)dataForLocalMemberLinkInString:(NSString *)string searchRange:(NSRange)searchRange flags:(GBProcessingFlag)flags {
    ...
    GBMethodData *referencedObject = [[(GBClassData *)[self currentContext] methods] methodBySelector:selector];
    if (!referencedObject) return nil;
    self.lastReferencedObject = referencedObject;
...
- (GBCrossRefData *)dataForRemoteMemberLinkInString:(NSString *)string searchRange:(NSRange)searchRange flags:(GBProcessingFlag)flags {
    ...
    id referencedMember = [[(GBClassData *)referencedObject methods] methodBySelector:selector];
    if (!referencedMember) {
