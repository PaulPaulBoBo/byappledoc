...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object   change:(NSDictionary *)change  context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"maximumNumberOfLogFiles"]) {
        ...
        if (IS_GCD_AVAILABLE) {
            ...
            [self performSelector:@selector(deleteOldLogFiles)    onThread:[DDLog loggingThread]  withObject:nil
                    waitUntilDone:NO];
...
- (NSString *)logsDirectory {
    ...
    if(![[NSFileManager defaultManager] fileExistsAtPath:logsDir]) {
        NSError *err = nil;
        if(![[NSFileManager defaultManager] createDirectoryAtPath:logsDir                 withIntermediateDirectories:YES attributes:nil error:&err]) {
...
- (void)setMaximumFileSize:(unsigned long long)newMaximumFileSize {
    ...
    if (IS_GCD_AVAILABLE) {
        ...
        if ([NSThread currentThread] == loggingThread) {
            ...
            [self performSelector:@selector(lt_setMaximumFileSize:)    onThread:loggingThread  withObject:newMaximumFileSizeWrapper
                    waitUntilDone:NO];
        }
...
- (void)setRollingFrequency:(NSTimeInterval)newRollingFrequency {
    ...
    if (IS_GCD_AVAILABLE) {
        ...
        if ([NSThread currentThread] == loggingThread) {
            ...
            [self performSelector:@selector(lt_setRollingFrequency:)    onThread:loggingThread  withObject:newMaximumRollingFrequencyWrapper
                    waitUntilDone:NO];
        }
...
- (void)scheduleTimerToRollLogFileDueToAge {
    ...
    rollingTimer = [[NSTimer scheduledTimerWithTimeInterval:[logFileRollingDate timeIntervalSinceNow]                                target:self                              selector:@selector(maybeRollLogFileDueToAge:)                              userInfo:nil                               repeats:NO] retain];
}
...
- (void)maybeRollLogFileDueToSize {
    ...
    if (fileSize >= maximumFileSize) {
        // YES, we are using direct access. Read note above.
